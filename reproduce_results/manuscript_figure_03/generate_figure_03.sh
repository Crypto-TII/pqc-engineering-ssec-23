...
cd ../../c-code
echo "cmake -DCMAKE_BUILD_TYPE=Release -DBENCHMARKING=CYCLES -DARCHITECTURE=x8664 -B cmake-build-release-cycles-x8664-graph-03"
cmake -DCMAKE_BUILD_TYPE=Release -DBENCHMARKING=CYCLES -DARCHITECTURE=x8664 -B cmake-build-release-cycles-x8664-graph-03
echo "cd cmake-build-release-cycles-x8664-graph-03"
cd cmake-build-release-cycles-x8664-graph-03
echo "make -j"
make -j

echo "benchmarks/benchmarks-ssec-p254 | tee benchmarks_ssec-p254-output.txt"
benchmarks/benchmarks-ssec-p254 | tee benchmarks_ssec-p254-output.txt

echo "benchmarks/benchmarks-ssec-p255 | tee benchmarks_ssec-p255-output.txt"
...
echo "benchmarks/benchmarks-ssec-p383 | tee benchmarks_ssec-p383-output.txt"
...
echo "benchmarks/benchmarks-ssec-p511 | tee benchmarks_ssec-p511-output.txt"
